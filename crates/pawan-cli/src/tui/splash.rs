//! Animated welcome splash (sine-wave field), inspired by maki-ui.

use super::theme::{self, extract_rgb, lerp_u8};
...
const TAGLINE: &str = "self-healing CLI coding agent";

const HELP_SEGMENTS: &[(&str, bool)] = &[
    ("Ctrl+P", true),
    (" fuzzy commands", false),
    ("  ·  ", false),
    ("Ctrl+L", true),
    (" sessions", false),
];
...
const TIPS: &[&str] = &[
    "Ctrl+P to fuzzy search slash commands",
    "Ctrl+L to browse saved sessions",
    "Ctrl+K to search conversation",
    "↑↓ arrows for command history",
    "Tab to auto-complete commands",
    "/heal to auto-fix build errors",
    "Ctrl+C to clear input",
    "/sessions to manage saved sessions",
];
...
impl Splash {
    ...
    pub fn render(&self, area: Rect, buf: &mut Buffer, accent: Color) {
        ...
        fill_area(area, buf, theme.background);
        ...
        render_version(area, buf, fade, area.y, env!("CARGO_PKG_VERSION"));
    }
    ...
    fn render_tip(&self, area: Rect, buf: &mut Buffer, fade: f32, tip_y: u16, accent: Color) {
        ...
        let theme = theme::current();
        let bg = theme.background;
        let tip_rgb = extract_rgb(theme.warning, (249, 226, 175));
        let ac = extract_rgb(accent, (100, 140, 255));
        let bg_rgb = extract_rgb(bg, (15, 15, 25));

        let tip = TIPS[self.tip_idx];
        let total_width = (5 + tip.len()) as u16;
        let x_start = area.x + area.width.saturating_sub(total_width) / 2;
        ...
        let segments: &[(&str, Style)] = &[
            ...
            (tip, faded_style(bg_rgb, ac, 0.55 * fade, bg)),
        ];
...
fn fill_area(area: Rect, buf: &mut Buffer, bg: Color) {
...
fn render_version(area: Rect, buf: &mut Buffer, fade: f32, y: u16, version: &str) {
    if y >= area.y + area.height {
        return;
    }
    let theme = theme::current();
...
mod tests {
    ...
    fn splash_smoke() {
        ...
        term.draw(|f| {
            splash.render(f.area(), f.buffer_mut(), Color::Rgb(100, 140, 255));
        })
        .unwrap();
    }
}
